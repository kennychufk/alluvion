...
#include "alluvion/dg/serialize.hpp"
#include "alluvion/dg/spinlock.hpp"
#include "alluvion/dg/z_sort_table.hpp"

namespace alluvion {
namespace dg {

template <typename TF>
class CubicLagrangeDiscreteGrid : public DiscreteGrid<TF> {
 public:
  using super = DiscreteGrid<TF>;
  CubicLagrangeDiscreteGrid(std::string const& filename) { load(filename); }
  CubicLagrangeDiscreteGrid(AlignedBox3r<TF> const& domain,
                            std::array<unsigned int, 3> const& resolution)
      : DiscreteGrid<TF>(domain, resolution){};

  void save(std::string const& filename) const {
    auto out = std::ofstream(filename, std::ios::binary);
    serialize::write(*out.rdbuf(), super::m_domain);
    serialize::write(*out.rdbuf(), super::m_resolution);
    serialize::write(*out.rdbuf(), super::m_cell_size);
    serialize::write(*out.rdbuf(), super::m_inv_cell_size);
    serialize::write(*out.rdbuf(), super::m_n_cells);
    serialize::write(*out.rdbuf(), super::m_n_fields);
  ...
  void load(std::string const& filename) {
    ...
    if (!in.good()) {
      std::cerr << "ERROR: Discrete grid can not be loaded. Input file does "
                   "not exist!"
                << std::endl;
      return;
    }

    serialize::read(*in.rdbuf(), super::m_domain);
    serialize::read(*in.rdbuf(), super::m_resolution);
    serialize::read(*in.rdbuf(), super::m_cell_size);
    serialize::read(*in.rdbuf(), super::m_inv_cell_size);
    serialize::read(*in.rdbuf(), super::m_n_cells);
    serialize::read(*in.rdbuf(), super::m_n_fields);
  ...
  unsigned int addFunction(
      typename DiscreteGrid<TF>::ContinuousFunction const& func,
      bool verbose = false,
      typename DiscreteGrid<TF>::SamplePredicate const& pred = nullptr) {
    using namespace std::chrono;

    auto t0_construction = high_resolution_clock::now();

    auto n = Eigen::Matrix<unsigned int, 3, 1>::Map(super::m_resolution.data());
...
#pragma omp for schedule(static) nowait
      for (int l = 0; l < static_cast<int>(n_nodes); ++l) {
        ...
        if (!pred || pred(x))
          ...
          c = std::numeric_limits<TF>::max();

        if (verbose &&
            (++counter == n_nodes ||
             duration_cast<milliseconds>(high_resolution_clock::now() - t0)
                     .count() > 1000u)) {
          std::async(std::launch::async, [&]() {
            ...
            std::cout << "\r"
                      ...
                      << 100.0 * static_cast<TF>(counter) /
                             static_cast<TF>(n_nodes)
                      << "%";
            mutex.unlock();
          });
        }
    ...
    cells.resize(super::m_n_cells);
    for (auto l = 0u; l < super::m_n_cells; ++l) {
      auto k = l / (n[1] * n[0]);
      auto temp = l % (n[1] * n[0]);
      auto j = temp / n[0];
      auto i = temp % n[0];
    ...
    cell_map.resize(super::m_n_cells);
    std::iota(cell_map.begin(), cell_map.end(), 0u);

    if (verbose) {
      std::cout << "\rConstruction took " << std::setw(15)
                << static_cast<TF>(
                       duration_cast<milliseconds>(
                           high_resolution_clock::now() - t0_construction)
                           .count()) /
                       1000.0
                << "s" << std::endl;
    }

    return static_cast<unsigned int>(super::m_n_fields++);
  }

  std::size_t nCells() const { return super::m_n_cells; };
  TF interpolate(unsigned int field_id, Vector3r<TF> const& x,
                 Vector3r<TF>* gradient = nullptr) const {
    if (!super::m_domain.contains(x)) return std::numeric_limits<TF>::max();

    auto mi = (x - super::m_domain.min())
                  .cwiseProduct(super::m_inv_cell_size)
                  .template cast<unsigned int>()
                  .eval();
    if (mi[0] >= super::m_resolution[0]) mi[0] = super::m_resolution[0] - 1;
    if (mi[1] >= super::m_resolution[1]) mi[1] = super::m_resolution[1] - 1;
    if (mi[2] >= super::m_resolution[2]) mi[2] = super::m_resolution[2] - 1;
    auto i = super::multiToSingleIndex({{mi(0), mi(1), mi(2)}});
    auto i_ = m_cell_map[field_id][i];
    if (i_ == std::numeric_limits<unsigned int>::max())
      return std::numeric_limits<TF>::max();

    auto sd = super::subdomain(i);
    i = i_;
    auto d = sd.diagonal().eval();

    auto denom = (sd.max() - sd.min()).eval();
    auto c0 = Vector3r<TF>::Constant(2.0).cwiseQuotient(denom).eval();
    auto c1 = (sd.max() + sd.min()).cwiseQuotient(denom).eval();
    auto xi = (c0.cwiseProduct(x) - c1).eval();

    auto const& cell = m_cells[field_id][i];
    if (!gradient) {
      // auto phi = m_coefficients[field_id][i].dot(shape_function_(xi,
      // nullptr));
      auto phi = 0.0;
      ...
      for (auto j = 0u; j < 32u; ++j) {
        ...
        if (c == std::numeric_limits<TF>::max()) {
          return std::numeric_limits<TF>::max();
        }
        phi += c * N[j];
      }
    ...
    auto dN = Eigen::Matrix<TF, 32, 3>{};
    auto N = shape_function_(xi, &dN);

    // TEST
    // auto eps = 1.0e-6;
    // auto ndN = Matrix<TF, 32, 3>{};
  ...
  bool determineShapeFunctions(unsigned int field_id, Vector3r<TF> const& x,
    ...
    if (!super::m_domain.contains(x)) return false;
    ...
    auto denom = (sd.max() - sd.min()).eval();
    c0 = Vector3r<TF>::Constant(2.0).cwiseQuotient(denom).eval();
    auto c1 = (sd.max() + sd.min()).cwiseQuotient(denom).eval();
    auto xi = (c0.cwiseProduct(x) - c1).eval();
  ...
  void reduceField(unsigned int field_id,
                   typename DiscreteGrid<TF>::Predicate pred) {
    ...
    for (auto l = 0u; l < coeffs.size(); ++l) {
      ...
      keep[l] =
          pred(xi, coeffs[l]) && coeffs[l] != std::numeric_limits<TF>::max();
    }
    ...
    for (auto l = 0u; l < coeffs.size(); ++l) {
      auto xi = indexToNodePosition(l);
      z_values[l] = zValue(xi, 4.0 * super::m_inv_cell_size.minCoeff());
    }
  ...
  void forEachCell(unsigned int field_id,
                   std::function<void(unsigned int, AlignedBox3r<TF> const&,
                                      unsigned int)> const& cb) const {
    auto n = super::m_resolution[0] * super::m_resolution[1] *
             super::m_resolution[2];
    for (auto i = 0u; i < n; ++i) {
      auto domain = AlignedBox3r<TF>{};
      auto mi = super::singleToMultiIndex(i);
      using namespace Eigen;
      domain.min() = super::m_domain.min() +
                     Eigen::Matrix<unsigned int, 3, 1>::Map(mi.data())
                         .template cast<TF>()
                         .cwiseProduct(super::m_cell_size);
      domain.max() = domain.min() + super::m_cell_size;
  ...
  // Data getters
  std::vector<std::vector<TF>>& node_data() { return m_nodes; }
  std::vector<std::vector<TF>> const& node_data() const { return m_nodes; }
  ...
  Vector3r<TF> indexToNodePosition(unsigned int l) const {
    auto x = Vector3r<TF>{};
    ...
    if (l < nv) {
      ...
      x = super::m_domain.min() +
          super::m_cell_size.cwiseProduct(ijk.template cast<TF>());
    } else if (l < nv + 2 * ne_x) {
      l -= nv;
      auto e_ind = l / 2;
      ...
      x = super::m_domain.min() +
          super::m_cell_size.cwiseProduct(ijk.template cast<TF>());
      x(0) += (1.0 + static_cast<TF>(l % 2)) / 3.0 * super::m_cell_size[0];
      ...
      x = super::m_domain.min() +
          super::m_cell_size.cwiseProduct(ijk.template cast<TF>());
      x(1) += (1.0 + static_cast<TF>(l % 2)) / 3.0 * super::m_cell_size[1];
    } else {
      l -= (nv + 2 * (ne_x + ne_y));
      auto e_ind = l / 2;
      ...
      x = super::m_domain.min() +
          super::m_cell_size.cwiseProduct(ijk.template cast<TF>());
      x(2) += (1.0 + static_cast<TF>(l % 2)) / 3.0 * super::m_cell_size[2];
    }

    return x;
  }

  static Eigen::Matrix<TF, 32, 1> shape_function_(
      Vector3r<TF> const& xi, Eigen::Matrix<TF, 32, 3>* gradient = nullptr) {
    // {{{
    auto res = Eigen::Matrix<TF, 32, 1>{};
  ...
  static inline uint64_t zValue(Vector3r<TF> const& x, TF invCellSize) {
    std::array<int, 3> key;
    ...
    std::array<unsigned int, 3> p = {
        static_cast<unsigned int>(static_cast<int64_t>(key[0]) -
                                  (std::numeric_limits<int>::lowest() + 1)),
        static_cast<unsigned int>(static_cast<int64_t>(key[1]) -
                                  (std::numeric_limits<int>::lowest() + 1)),
        static_cast<unsigned int>(static_cast<int64_t>(key[2]) -
                                  (std::numeric_limits<int>::lowest() + 1))};

    return morton_lut(p);
  }

  std::vector<std::vector<TF>> m_nodes;
