...
class BoundingSphere {
 public:
   ...
   */
  BoundingSphere(const Vector3r& x, F r) : m_x(x), m_r(r) {}
  ...
  void setPoints(const std::vector<Vector3r>& p) {
    ...
    v.erase(std::unique(v.begin(), v.end(),
                        [](Vector3r& a, Vector3r& b) { return a.isApprox(b); }),
            v.end());
    ...
    const F epsilon = 1.0e-6;
    for (int i = n - 1; i > 0; i--) {
      const Vector3r epsilon_vec = epsilon * Vector3r::Random();
      const int j = static_cast<int>(floor(i * F(rand()) / RAND_MAX));
      d = v[i] + epsilon_vec;
      v[i] = v[j] - epsilon_vec;
      v[j] = d;
    }
 ...
 private:
   ...
   */
  BoundingSphere ses3(int n, std::vector<Vector3r>& p, Vector3r& q1,
                      Vector3r& q2, Vector3r& q3) {
   ...
   * \return		smallest enclosing sphere
   */
  BoundingSphere ses2(int n, std::vector<Vector3r>& p, Vector3r& q1,
                      Vector3r& q2) {
