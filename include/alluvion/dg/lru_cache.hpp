#ifndef ALLUVION_DG_LRU_CACHE_HPP
#define ALLUVION_DG_LRU_CACHE_HPP
...
class LRUCache {
 public:
  ...
  LRUCache(eval_func_type const& f, std::size_t c) : _fn(f), _capacity(c) {
    assert(_capacity != 0);
  }
  ...
  value_type operator()(const key_type& k) {
    ...
    if (it == _key_to_value.end()) {
      ...
      _key_tracker.splice(_key_tracker.end(), _key_tracker,
                          (*it).second.second);
 ...
 private:
  ...
  void insert(key_type const& k, value_type const& v) {
    ...
    if (_key_to_value.size() == _capacity) evict();
