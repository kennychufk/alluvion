...
SCENARIO("testing index-to-node") {
  GIVEN("A valid domain and resolution") {
    AlignedBox3r domain(Vector3r(-23.1, 12.23, -31.44),
                        Vector3r(45.1, 103.2, 12.4));
    ...
    WHEN("composing a grid") {
      ...
      grid.addFunction([](Vector3r const& xi) { return xi(0); });
      ...
      THEN(
        ...
        for (U l = 0; l < num_nodes; ++l) {
          Vector3r host_position = grid.indexToNodePosition(l);
