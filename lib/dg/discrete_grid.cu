...
DiscreteGrid::MultiIndex DiscreteGrid::singleToMultiIndex(
...
unsigned int DiscreteGrid::multiToSingleIndex(MultiIndex const& ijk) const {
  return m_resolution[1] * m_resolution[0] * ijk[2] + m_resolution[0] * ijk[1] +
         ijk[0];
}

AlignedBox3r DiscreteGrid::subdomain(MultiIndex const& ijk) const {
  auto origin =
      m_domain.min() + Map<Matrix<unsigned int, 3, 1> const>(ijk.data())
                           .cast<F>()
                           .cwiseProduct(m_cell_size);
  return {origin, origin + m_cell_size};
}

AlignedBox3r DiscreteGrid::subdomain(unsigned int l) const {
