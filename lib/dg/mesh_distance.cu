...
MeshDistance::MeshDistance(TriangleMesh const& mesh, bool precompute_normals)
  ...
  m_cache.resize(
      max_threads,
      FunctionValueCache([&](Vector3r const& xi) { return signedDistance(xi); },
                         10000u));
  m_ucache.resize(
      max_threads,
      FunctionValueCache([&](Vector3r const& xi) { return distance(xi); },
                         10000u));
  ...
  if (m_precomputed_normals) {
    ...
    std::transform(
        m_mesh.faces().begin(), m_mesh.faces().end(), m_face_normals.begin(),
        [&](std::array<unsigned int, 3> const& face) {
          ...
          auto alpha = Vector3r{std::acos(e1.dot(-e3)), std::acos(e2.dot(-e1)),
                                std::acos(e3.dot(-e2))};
...
// Thread-safe.
F MeshDistance::distance(Vector3r const& x, Vector3r* nearest_point,
                         unsigned int* nearest_face, NearestEntity* ne) const {
  using namespace std::placeholders;

  auto dist_candidate = std::numeric_limits<F>::max();
  auto f = m_nearest_face[omp_get_thread_num()];
  if (f < m_mesh.nFaces()) {
    auto t =
        std::array<Vector3r const*, 3>{&m_mesh.vertex(m_mesh.faceVertex(f, 0)),
...
bool MeshDistance::predicate(unsigned int node_index,
                             TriangleMeshBSH const& bsh, Vector3r const& x,
                             F& dist_candidate) const {
...
void MeshDistance::callback(unsigned int node_index, TriangleMeshBSH const& bsh,
                            Vector3r const& x, F& dist_candidate) const {
...
F MeshDistance::signedDistance(Vector3r const& x) const {
  unsigned int nf;
...
F MeshDistance::signedDistanceCached(Vector3r const& x) const {
  return m_cache[omp_get_thread_num()](x);
}

F MeshDistance::unsignedDistance(Vector3r const& x) const {
  return distance(x);
}

F MeshDistance::unsignedDistanceCached(Vector3r const& x) const {
  return m_ucache[omp_get_thread_num()](x);
}

Vector3r MeshDistance::face_normal(unsigned int f) const {
...
Vector3r MeshDistance::edge_normal(Halfedge const& h) const {
...
Vector3r MeshDistance::vertex_normal(unsigned int v) const {
