...
struct HalfedgeEqualTo {
  ...
  bool operator()(Halfedge const& a, Halfedge const& b) const {
    ...
    std::array<unsigned int, 2> va = {(*faces)[fa][ea],
                                      (*faces)[fa][(ea + 1) % 3]};

    unsigned int fb = b.face();
    unsigned int eb = b.edge();
    std::array<unsigned int, 2> vb = {(*faces)[fb][eb],
                                      (*faces)[fb][(eb + 1) % 3]};
...
TriangleMesh::TriangleMesh(F const* vertices, unsigned int const* faces,
...
Vector3r TriangleMesh::computeFaceNormal(unsigned int f) const {
