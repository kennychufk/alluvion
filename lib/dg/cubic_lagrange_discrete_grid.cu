...
Matrix<F, 32, 1> shape_function_(Vector3r const &xi,
                                 Matrix<F, 32, 3> *gradient = nullptr) {
  auto res = Matrix<F, 32, 1>{};
...
inline uint64_t zValue(Vector3r const &x, F invCellSize) {
  std::array<int, 3> key;
...
Vector3r CubicLagrangeDiscreteGrid::indexToNodePosition(unsigned int l) const {
  auto x = Vector3r{};
  ...
  if (l < nv) {
    ...
    x = m_domain.min() + m_cell_size.cwiseProduct(ijk.cast<F>());
  } else if (l < nv + 2 * ne_x) {
    l -= nv;
    auto e_ind = l / 2;
    ...
    x = m_domain.min() + m_cell_size.cwiseProduct(ijk.cast<F>());
    x(0) += (1.0 + static_cast<F>(l % 2)) / 3.0 * m_cell_size[0];
    ...
    x = m_domain.min() + m_cell_size.cwiseProduct(ijk.cast<F>());
    x(1) += (1.0 + static_cast<F>(l % 2)) / 3.0 * m_cell_size[1];
  } else {
    l -= (nv + 2 * (ne_x + ne_y));
    auto e_ind = l / 2;
    ...
    x = m_domain.min() + m_cell_size.cwiseProduct(ijk.cast<F>());
    x(2) += (1.0 + static_cast<F>(l % 2)) / 3.0 * m_cell_size[2];
  }

  return x;
}

CubicLagrangeDiscreteGrid::CubicLagrangeDiscreteGrid(
    std::string const &filename) {
  load(filename);
}

CubicLagrangeDiscreteGrid::CubicLagrangeDiscreteGrid(
    AlignedBox3r const &domain, std::array<unsigned int, 3> const &resolution)
...
unsigned int CubicLagrangeDiscreteGrid::addFunction(
    ContinuousFunction const &func, bool verbose, SamplePredicate const &pred) {
      ...
      if (verbose &&
        ...
        std::async(std::launch::async, [&]() {
          ...
          std::cout << "\r"
                    ...
                    << 100.0 * static_cast<F>(counter) / static_cast<F>(n_nodes)
                    << "%";
          mutex.unlock();
        });
      }
...
bool CubicLagrangeDiscreteGrid::determineShapeFunctions(
    unsigned int field_id, Vector3r const &x,
    std::array<unsigned int, 32> &cell, Vector3r &c0,
    Eigen::Matrix<F, 32, 1> &N, Eigen::Matrix<F, 32, 3> *dN) const {
  if (!m_domain.contains(x)) return false;

  auto mi = (x - m_domain.min())
                .cwiseProduct(m_inv_cell_size)
                .cast<unsigned int>()
                .eval();
...
F CubicLagrangeDiscreteGrid::interpolate(
    unsigned int field_id, Vector3r const &xi,
    const std::array<unsigned int, 32> &cell, const Vector3r &c0,
    const Eigen::Matrix<F, 32, 1> &N, Vector3r *gradient,
    Eigen::Matrix<F, 32, 3> *dN) const {
  if (!gradient) {
    auto phi = 0.0;
    for (auto j = 0u; j < 32u; ++j) {
      auto v = cell[j];
      auto c = m_nodes[field_id][v];
      if (c == std::numeric_limits<F>::max()) {
...
F CubicLagrangeDiscreteGrid::interpolate(unsigned int field_id,
                                         Vector3r const &x,
                                         Vector3r *gradient) const {
  if (!m_domain.contains(x)) return std::numeric_limits<F>::max();
  ...
  auto dN = Matrix<F, 32, 3>{};
  auto N = shape_function_(xi, &dN);

  // TEST
  // auto eps = 1.0e-6;
  // auto ndN = Matrix<F, 32, 3>{};
...
void CubicLagrangeDiscreteGrid::reduceField(unsigned int field_id,
                                            Predicate pred) {
  ...
  for (auto l = 0u; l < coeffs.size(); ++l) {
    ...
    keep[l] = pred(xi, coeffs[l]) && coeffs[l] != std::numeric_limits<F>::max();
  }
...
void CubicLagrangeDiscreteGrid::forEachCell(
    unsigned int field_id,
    std::function<void(unsigned int, AlignedBox3r const &, unsigned int)> const
        &cb) const {
  ...
  for (auto i = 0u; i < n; ++i) {
    ...
    domain.min() =
        m_domain.min() +
        Matrix<unsigned int, 3, 1>::Map(mi.data()).cast<F>().cwiseProduct(
            m_cell_size);
