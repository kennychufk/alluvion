...
#include "alluvion/dg/volume_integration.hpp"
...
VolumeIntegration::VolumeIntegration(
  ...
  for (unsigned int i(0); i < m_nFaces; ++i) {
    const Vector3r &a = m_vertices[m_indices[i][0]];
    const Vector3r &b = m_vertices[m_indices[i][1]];
    const Vector3r &c = m_vertices[m_indices[i][2]];
...
void VolumeIntegration::compute_inertia_tensor(F density) {
  volume_integrals();
  m_volume = static_cast<F>(T0);

  m_mass = static_cast<F>(density * T0);

  /* compute center of mass */
  m_r[0] = static_cast<F>(T1[0] / T0);
  m_r[1] = static_cast<F>(T1[1] / T0);
  m_r[2] = static_cast<F>(T1[2] / T0);

  /* compute inertia tensor */
  m_theta(0, 0) = static_cast<F>(density * (T2[1] + T2[2]));
  m_theta(1, 1) = static_cast<F>(density * (T2[2] + T2[0]));
  m_theta(2, 2) = static_cast<F>(density * (T2[0] + T2[1]));
  m_theta(0, 1) = m_theta(1, 0) = -density * static_cast<F>(TP[0]);
  m_theta(1, 2) = m_theta(2, 1) = -density * static_cast<F>(TP[1]);
  m_theta(2, 0) = m_theta(0, 2) = -density * static_cast<F>(TP[2]);
...
void VolumeIntegration::projection_integrals(unsigned int f) {
  F a0, a1, da;
  F b0, b1, db;
  F a0_2, a0_3, a0_4, b0_2, b0_3, b0_4;
  F a1_2, a1_3, b1_2, b1_3;
  F C1, Ca, Caa, Caaa, Cb, Cbb, Cbbb;
  F Cab, Kab, Caab, Kaab, Cabb, Kabb;
