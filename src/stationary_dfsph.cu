...
int main(void) {
  ...
  cnst::set_dfsph_factor_epsilon(1e-5);
  ...
  display->camera_.setEye(0.f, 19.45f, 35.5f);
  display->camera_.setCenter(0.f, -2.84f, 0.f);
  ...
  display->add_shading_program(new ShadingProgram(
      nullptr, nullptr, {}, {}, [&](ShadingProgram& program, Display& display) {
        ...
        for (U frame_interstep = 0; frame_interstep < 10; ++frame_interstep) {
          ...
          F avg_density_err = std::numeric_limits<F>::max();
          constexpr F kMaxError = 1e-3;
          ...
          while (num_divergence_solve_iteration < 1 ||
            ...
            avg_density_err = Runner::sum(particle_density_adv, num_particles) /
                              num_particles;
          ...
          avg_density_err = std::numeric_limits<F>::max();
          U num_pressure_solve_iteration = 0;
          while (num_pressure_solve_iteration < 2 ||
                 avg_density_err > kMaxError) {
            Runner::launch(num_particles, 256, [&](U grid_size, U block_size) {
            ...
            avg_density_err =
                Runner::sum(particle_density_adv, num_particles) /
                    num_particles -
          ...
          // ===== ]pressure solve
          Runner::launch(num_particles, 256, [&](U grid_size, U block_size) {
            integrate_velocity<<<grid_size, block_size>>>(
                particle_x, particle_v, dt, num_particles);
          });

          Runner::launch(num_particles, 256, [&](U grid_size, U block_size) {
            scale<<<grid_size, block_size>>>(particle_kappa_v,
                                             particle_normalized_attr, -0.002_F,
                                             0.0_F, num_particles);
          });
